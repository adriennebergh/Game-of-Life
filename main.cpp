...
int main(int argc, char **argv) {
	...
	Cell c;
	bool outT;
	cout << "Would you like to enter a map file, or be provided a random configuration? (map/random)" << endl;
	...
	if(response == "map") {
		...
		while(getline(cin,boardType)) {
			if (boardType == "mirror") {
				c.numNeighborsMirror(board, boardB, width, height, flipType, outT, 0);
				break;
			}
			if(boardType == "classic") {
				c.numNeighborsClassic(board, boardB, width, height, flipType, outT, 0);
				break;
			}
			if(boardType == "donut") {
				c.numNeighborsDonut(board, boardB, width, height, flipType, outT, 0);
				break;
			}
		}
